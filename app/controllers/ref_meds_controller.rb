class RefMedsController < ApplicationController
  ...
  def create
    respond_to do |format|
        format.html do
          r = Referral.find(session[:referral]) 
          redirect_to r, notice: 'Ref med was successfully created.'
        end
        format.json do 
          if params[:ref_med][:id].present? 
            ...
            @ref_med.referral_id = session[:referral] if session[:referral].present? 
            @ref_med.save
          end 
          render :show, status: :created, location: @ref_med  
        end  
    end
  end
